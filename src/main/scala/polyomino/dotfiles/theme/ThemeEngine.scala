...
@define-color rosewater ${palette.rosewater};"""
    ...
    val waybarStyleContent =
      s"""@import "${ctx.configDir / "polyomino" / "theme" / "tokens.css"}";
        ...
        |}
        |
        |/* Waybar Monolith Header Bar matching Lockscreen & Power Menu TUI identity */
        |window#waybar {
        ...
        |}
        |
        |/* Clean workspace indicators with sharp 1px bottom accent */
        |#workspaces button {
        |    min-width: 20px;
        |    padding: 0 10px;
        |    margin: 0 2px;
    ...
    val fuzzelContent =
      s"""# Generated by polyomino.dotfiles theme engine — flavor=${palette.name}
         ...
         |selection=${accentClean}ff
         |selection-text=${baseClean}ff
         |selection-match=${baseClean}ff
