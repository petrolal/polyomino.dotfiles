...
@define-color blue ${palette.blue};"""
    ...
    val swayColorsContent =
      s"""# Generated by polyomino dotfiles theme engine — flavor=${palette.name} mode=$mode
         ...
         |""".stripMargin
    val isSwayfx = try {
      val v = os.proc("sway", "--version").call(check = false).out.text().toLowerCase
      v.contains("swayfx")
    } catch {
    ...
    val swayFxContent = if isSwayfx then
      ...
      """# Generated by polyomino dotfiles theme engine — Standard Sway active
        |# SwayFX-specific effects omitted to prevent config validation errors
        |""".stripMargin
