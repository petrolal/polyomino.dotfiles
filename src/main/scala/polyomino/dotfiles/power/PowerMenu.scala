...
/** `power-menu` — a Tetris-themed power / exit modal.
  ...
  *
  * Controls: Left/Right or `h`/`l` switch lanes; Enter / Space / Down hard-drop
  * (run the selected action immediately); `Esc` cancels and runs nothing. If the
...
object PowerMenu:
  ...
  private val HelpText: String =
    """polyomino power-menu — Tetris-themed power / exit modal.
      ...
      |  enter / space / down  drop now (run the selected action immediately)
